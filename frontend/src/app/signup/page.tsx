...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.status === 201) {
        // Automatically logged in by backend (if it works like that), or we can just redirect to login
        router.push('/login')
      } else if (res.status === 202) {
        const data = await res.json()
        setMessage(data.detail || 'Đăng ký thành công. Vui lòng kiểm tra email để xác nhận.')
      } else {
        const data = await res.json()
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/20 rounded-2xl p-8 shadow-2xl shadow-black/20">
        ...
        {message && (
          <div className="bg-green-500/10 border border-green-500/50 text-green-200 p-3 rounded-lg text-sm mb-6">
            {message}
          </div>
        )}
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          >
            {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><UserPlus className="w-5 h-5" /> Đăng ký</>}
          </button>
        </form>
